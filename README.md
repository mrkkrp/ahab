...
```starlark
bazel_dep(name = "ahab", version = "0.3.0")
```
...
A script the build expands from a template is named by the template, when a
...
```
@rules_rs+toolchains//*/bin/rustc
```

Most programs need no such thing, because what varies between machines and
releases sits in the repository name and is normalized away before a
specification ever sees it. Some rule sets put it in the path instead—that
one matches `rustc/default_linux_x86_64_1_86_0_rust_toolchain/bin/rustc`,
and an exact name would have had to state both the platform and the Rust
release, and would have stopped matching at the next bump of either.
