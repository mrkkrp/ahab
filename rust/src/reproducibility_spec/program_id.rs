...
impl ProgramId {
    ...
    /// The same program, attributed to `module`.
    pub fn in_module(self, module: &str) -> ProgramId {
        let Origin::Main { extension } = self.origin else {
            return self;
        };
        ProgramId {
            origin: Origin::Module {
...
mod tests {
    ...
    #[test]
    fn naming_the_module_attributes_the_repository_under_analysis() {
        assert_eq!(
            ProgramId::of("bazel-out/k8-fastbuild/bin/tools/gen")
                .in_module("rules_pkg"),
            ProgramId::module("rules_pkg", "tools/gen"),
        );
    }

    #[test]
    fn an_extension_of_the_repository_under_analysis_is_attributed_too() {
        assert_eq!(
            ProgramId::of("external/+typescript+npm_typescript/tsc_/tsc")
                .in_module("aspect_rules_ts"),
            ProgramId::extension(
                "aspect_rules_ts",
                "typescript",
                "tsc_/tsc",
    ...
    #[test]
    fn a_program_that_already_names_a_repository_is_left_alone() {
        for id in [
            ProgramId::module("rules_rust", "util/pw"),
            ProgramId::extension("llvm", "t", "bin/clang"),
            ProgramId::of("/usr/bin/gcc"),
        ] {
            assert_eq!(id.clone().in_module("rules_pkg"), id);
        }
    }
